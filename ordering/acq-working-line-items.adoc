...
or backordered, but the library is still expecting to a copy. This enables you to track 
...
[NOTE]
======
If line items are being delayed for different reasons they will need to be delayed 
separately so different cancel reasons can be used.
======
+
. Click *Apply*.
+
...
+
. The status of the line item becomes _Delayed_ and the colour changes to blue.
+
