...
If something happens to an entire order, such as a shipment is lost, the purchase order
can be delayed in Evergreen.  This enables you to track what is going on with the 
...
Delaying a purchase order uses the same functionality as cancelling it. The crucial 
...
.. You must choose a reason that begins with *Delayed*.
. Click *Apply*.
+
...
+
. The status of the purchase order becomes and all line items becomes _Delayed_.
The colour of the line items changes to blue.
+
...
.. A delayed purchase order is resolved when all the items are marked as received or cancelled.


Cancelling a Purchase Order
~~~~~~~~~~~~~~~~~~~~~~~~~~~

If an order has been cancelled by the library or your provider and you no longer
expect to receive the order the entire purchase 
can be cancelled.  To cancel an entire purchase orders all line items must have the 
...
+
. In the pop-up that appears choose a reason from the drop down menu.
.. You must choose a reason that begins with *Canceled*.
. Click *Apply*.
+
...
the line items changes to white.
+
