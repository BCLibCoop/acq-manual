...
. xref:_propagation_only[] - Create analogous funds for the next year.

. xref:_rollover_encumbrances_only[] - Create analogous funds for the next year and rollover encumbrances.  Unspent money is returned to the funding source.

. xref:_rollover_encumbrances_and_unspent_money[] - Create analogous funds for the next year and rollover encumbrances and unspent money.

. xref:_rollover_encumbrances_and_unspent_money_for_specific_funds[] - Create analogous funds for the next year and rollover encumbrances and unspent money for some funds and only encumbrances for other funds.
...
[NOTE]
The Limit Fiscal Year Close-out Operation to Encumbrances will only display if the setting 8Allow funds to be rolled over without bringing the money along* is set to *True* in the Library Settings Editor.
